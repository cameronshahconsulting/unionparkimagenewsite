...
export async function sendInstallEstimateEmails({
  ...
  const customerHtml = `<div style="max-width:560px;margin:0 auto;padding:8px 4px;font-family:system-ui,sans-serif;color:#0e2730">
<p style="font-size:22px;font-weight:800;color:#093544;margin:0 0 12px">We’ll be in touch to schedule your estimate</p>
...
${addr ? `<p style="font-size:14px;line-height:1.45;margin:0 0 12px;color:#4a6570">Install address: ${String(addr).replace(/</g, "")}${milesNote}</p>` : ""}
${plantSubtotal ? `<p style="font-size:14px;margin:0 0 8px">Plant subtotal on this plan: about ${money(plantSubtotal)}</p>` : ""}
${plantSummary ? `<pre style="white-space:pre-wrap;font-size:13px;background:#ecf8fc;padding:12px;border-radius:8px;color:#4a6570">${String(plantSummary).replace(/</g, "")}</pre>` : ""}
