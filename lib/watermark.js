...
function svgBar(width) {
  ...
  return Buffer.from(`
    <svg width="${width}" height="${barHeight}" xmlns="http://www.w3.org/2000/svg">
      <rect x="0" y="0" width="${width}" height="${barHeight}" fill="rgb(6,39,51)" />
      <text x="12" y="${Math.round(barHeight / 2 + fontSize / 3)}" font-family="system-ui, -apple-system, sans-serif" font-size="${fontSize}" fill="#cfeef7" font-weight="600">${LABEL}</text>
    </svg>
  `);
}
