 ...
 */
const LABEL = "AI-generated illustration · not a photograph · Annie's Online Nursery";
...
function svgBar(width) {
  ...
  return Buffer.from(`
    <svg width="${width}" height="${barHeight}" xmlns="http://www.w3.org/2000/svg">
      <rect x="0" y="0" width="${width}" height="${barHeight}" fill="rgb(20,28,16)" />
      <text x="12" y="${Math.round(barHeight / 2 + fontSize / 3)}" font-family="system-ui, -apple-system, sans-serif" font-size="${fontSize}" fill="#FFFBF1" font-weight="600">${LABEL}</text>
    </svg>
  `);
}
