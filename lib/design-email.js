...
export async function sendDesignReadyEmail({ email, designId, plan: planArg = null }) {
  ...
  const text = [
    `Your Union Park yard design is ready.`,
    ``,
    summary || null,
    summary ? `` : null,
...
<p style="font-family:system-ui,sans-serif;font-size:22px;font-weight:800;color:#093544;margin:0 0 10px">Your yard design is ready</p>
${summary ? `<p style="font-family:system-ui,sans-serif;font-size:16px;color:#4a6570;line-height:1.4;margin:0 0 16px">${summary.replace(/</g, "")}</p>` : ""}
...
<p style="margin:0 0 18px"><a href="${link}" style="display:inline-block;background:#093544;color:#ffffff;font-family:system-ui,sans-serif;font-weight:800;font-size:14px;text-decoration:none;padding:12px 22px;border-radius:8px">View your design</a></p>
${htmlItems}
${subtotal ? `<p style="font-family:system-ui,sans-serif;font-size:14px;font-weight:800;color:#093544;margin:8px 0 0">Plant total about ${money(subtotal)}</p>` : ""}
...
<p style="font-family:system-ui,sans-serif;font-size:12px;color:#7a7162;margin:22px 0 0;line-height:1.4">If the button doesn’t work, paste this link into your browser:<br/><a href="${link}" style="color:#0f6884">${link}</a></p>
<p style="font-family:system-ui,sans-serif;font-size:12px;color:#7a7162;margin:16px 0 0">— Union Park Landscaping · Wilmington, DE</p>
${compliance.html}
</div>`;
  ...
  try {
    await sendEmail({
      to: email,
      subject: "Your Union Park yard design is ready",
      text,
      html,
    });
...
/** Ping the UPL inbox when someone generates a yard design. */
